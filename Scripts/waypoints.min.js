...
function numberAnimationIncrease(elem) {
    jQuery(elem+" .ve_number_count").waypoint(function(direction){
            var el = jQuery(this); 
            var finish_number = jQuery(this).attr("data-number"); 
            var number = parseInt(finish_number.replace(" ", ""));
            if(number>0) {
                jQuery({val: 0}).animate({val: number}, {
                    duration: 2000,
                    easing:"swing", 
                    step: function() { 
                        jQuery('span',el).text(Math.floor(this.val+1).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " "));
                    },
                    complete: function() {
                        jQuery('span',el).html(finish_number);
                    }
                });  
            }
        },{ 
